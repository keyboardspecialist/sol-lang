...
Completed typed IR is ownership-checked in a separate transactional pass before interpretation. Local reads are explicitly `COPY`, `MOVE`, or narrowly non-consuming `RECEIVER` uses; the validator independently recomputes and checks those annotations. `Int64`, `Bool`, `Text`, `Unit`, `Never`, `Option`/`Result` with copyable arguments, and nongeneric records/enums/distinct/refined values with recursively copyable contents use the bootstrap structural `Copy` rule. Capabilities, functions, bound operations, `Self`, generic parameters, and generic nominal values are affine. Moves consume the whole local, and `SOL-OWNERSHIP-001` reports later use. `if`, `match`, and short-circuit paths join by retaining a local only when every continuing path retains it; terminating paths do not constrain continuation. Direct capability operation receivers and direct handler authority/provider setup remain non-consuming so repeated direct capability operations work, but capability aliases and bound-operation aliases move by value. There are no partial moves, borrows, loops, or drop/cleanup semantics yet. The interpreter deep-clones copy/receiver reads and transfers move reads out of their frame slots.
