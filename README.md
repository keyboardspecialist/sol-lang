...
The bootstrap exposes a shared compilation session for file, directory, NUL-terminated text, and length-delimited in-memory source compilation. Frontend package and semantic arenas remain private: callers receive only copied scalar summaries and diagnostic records, borrowed strings whose lifetime ends at IR transfer or session destruction, and session-owned diagnostic/inspection render operations. Successful compilation can transfer one opaque validated-IR handle that remains self-contained after session destruction. Narrow read-only definition/path projections, effects rendering, and interpretation never expose its internal `SolIr`. Validated-handle interpretation rejects raw-IR host callbacks before execution; E3 will define the safe host profile. The compiler-internal reference interpreter consumes validated IR only after all frontend data is freed. It executes the bounded immutable core, explicit trait evidence, root-preserving capabilities, and exact deep handlers under deterministic resource limits. `sol test` runs authored Boolean unit tests through that interpreter. It is not a production runtime: there is no `sol run`, VM, bytecode, code generation, or runtime contract enforcement. Contracts are retained as IR templates but ignored during test execution. The raw IR and interpreter APIs, including raw host callbacks, remain explicitly unstable compiler internals for phase and malformed-input tests, not serialized formats or stable public ABIs.
