...
The production track is underway. P1, P2.1, and P2.2 are complete for the frozen
E6 profile; P2.3 is the next open checkpoint:
...
| [ ] | P2.3b2 | Complete concrete CFG, dispatch, handlers, and writeback | Replace symbolic blocks, terminators, edges, parameter/edge values, loops, and every deferred instruction/operand payload with complete concrete records; add explicit handler source/provider bindings and frame operations, direct/indirect dispatch, normal-edge exclusive writeback, and any newly discovered callable specializations to a deterministic fixed point |
| [ ] | P2.3b3 | Revalidate complete concrete dataflow | Independently validate concrete SSA dominance, affine ownership, moves/borrows/transports, cleanup on every exit, contracts/refinements, handler balance, and full E6 closure after P2.3b2 establishes one complete concrete executable vocabulary |
