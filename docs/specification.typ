...
- Separate P1a.1-P1a.3a callable-scoped target-neutral CFG MIR checkpoints for validated nongeneric free/test functions, with deterministic blocks, SSA instruction values/block parameters over explicit local storage, scalar and whole-local operations, conditional cleanup/storage death, `if`, short-circuit Boolean control, return, panic, lexical regions, `require`, proof-backed unreachable, and statement-form `loop`/`while` with nearest-loop break/continue. Nongeneric direct function calls retain ordered owned and call-scoped whole-local shared/exclusive operands in abstract invoke terminators with distinct normal/failure-cleanup edges and normal-only exclusive writeback. Loop metadata binds preheaders, condition/body/exit edges, natural backedges, nested transfer identity, cleanup boundaries, and erased proof-obligation slices. Transactional unsupported lowering and independent arena, edge, source/type/control-event, invoke provenance/result, initialization/move, lexical cleanup-order, region-stack, and cyclic loop-CFG validation are operational. It is not yet a complete executable-core MIR or a compilation-session/backend input.
