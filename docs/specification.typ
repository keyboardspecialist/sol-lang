...
#status("IMPLEMENTED", [The bootstrap checks whole-local affine moves and callable-scoped shared/exclusive loans after typed IR lowering. Unqualified parameters are owned; `borrow T` is shared and `inout T` is exclusive access over underlying `T`. Borrowed operands must be direct local places and loans overlap through call completion. `SOL-OWNERSHIP-001` reports use after move, and `002` through `004` report borrow conflicts and invalid escapes. `region name { body }` is a Unit-valued statement, not an expression or first-class allocator value; labels are retained only for diagnostics/internal IR. Regions nest and have ordinary type/effect checking but are forbidden in contracts and refinements. Every executable block records exact successful `let` introduction order and every match arm records exact pattern-binding order. Live owned slots release in reverse on every lexical exit; moved slots are skipped, borrowed parameters are not cleanup owners, and Copy slots still release runtime storage. Affine explicit returns and affine propagation residuals from active explicit regions are conservatively rejected by `SOL-REGION-001`; Copy returns are allowed. Inspection v1 remains unchanged. First-class borrow/region values, borrowed `let` aliases, partial/field borrows, mutation, loops, allocator APIs, lifetime generics, and user resource cleanup remain deferred.])
