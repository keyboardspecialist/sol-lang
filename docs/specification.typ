...
#status("IMPLEMENTED", [Raw `sol_interpret` and its raw-IR host callback remain trusted compiler-test interfaces. Opaque validated-handle interpretation rejects every non-null host callback with `SOL_INTERPRETER_INVALID_REQUEST` before raw interpretation, so no private IR escapes. E3 defines the future safe host profile.])
