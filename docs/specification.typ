...
- An initial separate callable-scoped target-neutral CFG MIR checkpoint for validated nongeneric free/test functions, with deterministic blocks, SSA instruction values/block parameters over explicit local storage, scalar and whole-local operations, conditional cleanup/storage death, `if`, return, panic, transactional unsupported lowering, and independent structural/type/value/storage validation. It is not yet a complete executable-core MIR or a compilation-session/backend input.
