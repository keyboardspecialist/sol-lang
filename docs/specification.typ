...
#status("IMPLEMENTED", [The bootstrap checks path-sensitive affine moves, mutable replacement, and callable-scoped shared/exclusive loans after typed IR lowering. Operational projections are local-rooted record fields. Paths overlap when they share a root and either field-ID path prefixes the other; siblings diverging at a field are disjoint. Moving an affine field makes that path and its ancestors unavailable while preserving siblings. Exact field or ancestor assignment can restore availability. `place = rhs` context-checks and analyzes the RHS first, rejects overlapping loans, records UPDATE use metadata, cleans a live old leaf exactly once, and fills a moved hole without old-value cleanup. Projected mutation conservatively rejects capability-bearing, function/operation, `Self`, and unresolved-generic leaf types because field-sensitive authority provenance is deferred. Source-defined functions, callbacks, recursive forwarding, and implementation methods implement `inout` as copy-in/copy-out and write back only after successful completion; bodyless host operations and top-level interpreter entries reject exclusive writeback. All destination paths are metered before any staged writeback commits. Affine writeback across an explicit-region boundary is rejected by `SOL-REGION-001`. Index/dereference projections, uninitialized `var`, compound assignment, escaping references, loops, allocator APIs, lifetime generics, and user resource cleanup remain deferred. Inspection v1 is unchanged.])
