...
#spec-table(
  ...
  (
    ...
    ([`inout T` parameter], [Call-scoped exclusive access to a writable local-rooted record-field place; supported source callables copy in and write back on success.]),
...
#status("IMPLEMENTED", [The bootstrap accepts statement-only `loop { body }` and `while condition { body }`, plus payloadless final-in-block `break` and `continue` targeting the nearest loop. Conditions are `Bool`; bodies are Unit or locally terminating. `while` always retains a condition-false exit. Bare `loop` falls through only through reachable breaks. Ownership computes a finite fixed point over initial entry plus fallthrough/continue backedges, joining unavailable paths by union and definite initialization by intersection, then joins reachable break states for the post-loop path. Conditions execute inside the loop transfer context. Runtime blocks unwind lexical locals on fallthrough, continue, break, return, propagation, error, and step-limit failure, and unregister per-iteration bindings before the next visit. Labels, exit payloads, loop values, invariants, `decreases`, and proof/totality integration remain deferred.])
