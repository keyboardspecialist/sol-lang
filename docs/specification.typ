...
- Separate P1a.1-P1a.3b1 callable-scoped target-neutral CFG MIR checkpoints for validated nongeneric free/test functions, with deterministic blocks, SSA instruction values/block parameters over explicit local storage, scalar and whole-local operations, conditional cleanup/storage death, `if`, short-circuit Boolean control, return, panic, lexical regions, `require`, proof-backed unreachable, and statement-form `loop`/`while` with nearest-loop break/continue. Nongeneric direct function calls retain ordered owned and call-scoped whole-local shared/exclusive operands in abstract invoke terminators with distinct normal/failure-cleanup edges and normal-only exclusive writeback. Loop metadata binds preheaders, condition/body/exit edges, natural backedges, nested transfer identity, cleanup boundaries, and erased proof-obligation slices. Authority-free infallible zero/one-payload records, enum variants, Option/Result cases, and non-refined distinct wrappers use semantic construction instructions with exact tags and source-ordered operand metadata, without layout commitments. Transactional unsupported lowering and independent arena, edge, source/type/control-event, invoke/construction provenance, initialization/move, lexical cleanup-order, region-stack, and cyclic loop-CFG validation are operational. It is not yet a complete executable-core MIR or a compilation-session/backend input.

It is not a production compiler. In particular, general row constraints and explicit/multiple/recursive or authority-capturing effect parameters, refinement assumptions and refined patterns, richer traits and constrained or higher-order generics, first-class or escaping borrows, index/dereference mutation, field-by-field or authority-bearing initialization, loop labels/values and runtime obligation checking, first-class/allocator regions, lifetime generics, user-defined Drop/destructors/finalizers/close protocols, unsafe/FFI authority gates, general algebraic handlers, static/unparameterized or runtime-dynamic handler matching, call-site proof use, logical normalization, SMT/proof discharge, concurrency, package manifests/host wiring/dependencies, member/local stable identities, width-based and declaration-reordering formatting, stable public schemas/IR, semantic patches, backend cleanup behavior, and code generation are not implemented.
