...
- P2.1 adds an unstable symbolic `SolMirProgram` owner borrowing one validated owning IR. Deterministic entry/test/internal-fixture roots cache one MIR template per reachable lowerable bodyful callable, retain exact approved safe bodyless capability imports, propagate forwarded evidence only through compatible concrete incoming invocation contexts, record full typed concrete trait specialization demands, and preserve path-free invoke/static/handler/predicate provenance under local callable, reference, and discovery-work limits. Trait-bounded generic roots are rejected independently rather than borrowing evidence from another root. Each callback site accepts only an exact direct definition or bound-operation callee; current source/P1 ownership and lowering do not yet produce that first-class static form, so dynamic, conditional, and aliased forms reject. Input deduplication, prerequisite owning-IR validation, and nested public MIR lowering/validation are explicitly outside the local discovery meter. This is a trusted mutable compiler-internal owner, not a hostile arbitrary-pointer format: validation rejects noncanonical null-empty headers and detectable top-level/nested owned-range overlap before rebuilding the authentic closure, while teardown requires original builder allocation bases. Portable C does not prove allocator provenance for arbitrary adjacent/interior pointers. The owner performs no monomorphization, substitution, representation/layout, ABI, symbol, linkage, or backend work.
