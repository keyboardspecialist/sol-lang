...
- Separate P1a.1-P1a.2 callable-scoped target-neutral CFG MIR checkpoints for validated nongeneric free/test functions, with deterministic blocks, SSA instruction values/block parameters over explicit local storage, scalar and whole-local operations, conditional cleanup/storage death, `if`, short-circuit Boolean control, return, panic, lexical regions, `require`, and proof-backed unreachable. Nongeneric direct function calls retain ordered owned and call-scoped whole-local shared/exclusive operands in abstract invoke terminators with distinct normal/failure-cleanup edges and normal-only exclusive writeback. Transactional unsupported lowering and independent arena, edge, source/type/control-event, invoke provenance/result, initialization/move, lexical cleanup-order, and region-stack validation are operational. It is not yet a complete executable-core MIR or a compilation-session/backend input.
