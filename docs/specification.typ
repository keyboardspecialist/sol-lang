...
#status("IMPLEMENTED", [The bootstrap checks whole-local affine moves, mutable replacement, and callable-scoped shared/exclusive loans after typed IR lowering. Owning IR normalizes executable local and resolved-field accesses into canonical typed places with a local or computed-value root and flattened root-to-leaf field projections. `var name = initializer` infers one fixed slot type; `place = rhs` is a statement and currently accepts only an unprojected mutable binding-local root. The RHS is context-checked and analyzed first, then replacement rejects active loans, marks the slot available, and records explicit UPDATE use metadata. Affine self-assignment therefore moves and reinitializes successfully. Projected reads preserve whole-root COPY/MOVE semantics; projected assignment, partial moves/loans, and caller writeback remain deferred. Assignment preserves the initializer's exact normalized capability/operation provenance, adds only RHS effects, and is forbidden in contracts/refinements. `region name { body }` is a Unit-valued statement. Every executable block records exact successful `let`/`var` introduction order and every match arm records exact pattern-binding order; assignments add no cleanup entry. Live old slots receive deterministic replacement cleanup before transfer, while moved slots are reinitialized without old cleanup. Introduction explicit-region depth is tracked per local, and affine updates from deeper active explicit regions into shallower locals are conservatively rejected by `SOL-REGION-001`; Copy updates may cross. Inspection v1 adds only optional Boolean `mutable` on HIR locals. Field/index/dereference mutation, operational indexing/dereference, uninitialized `var`, compound assignment, inout caller writeback, loops, flow-sensitive authority provenance, allocator APIs, lifetime generics, and user resource cleanup remain deferred.])
