...
#status("IMPLEMENTED", [P1a.1-P1a.2 add a separate unstable `SolMir` owner for one selected validated nongeneric free/test callable. The target-neutral subset normalizes scalar literals and operations, whole-local copy/move/store, lexical storage and conditional cleanup, blocks, `if`, short-circuit Boolean control, return, panic, lexical regions, `require`, and proof-backed unreachable into deterministic basic blocks with SSA instruction values and block parameters. Nongeneric direct function calls use abstract invoke terminators with explicit normal/failure-cleanup edges and ordered owned/shared/exclusive operands; exclusive whole-local writeback occurs only on the normal edge. Independent validation checks arenas, edges, invoke result/provenance, source/type/control-event relations, local value availability, parameter activation, initialization/move transitions, lexical cleanup order, and balanced regions. Unsupported forms fail transactionally. The production compilation session and interpreter do not consume MIR; loops, patterns, propagation, aggregates, projected places, handlers, dynamic/method/capability calls, contracts, generic lowering, layout, ABI, and backend execution remain P1a.3+.])
