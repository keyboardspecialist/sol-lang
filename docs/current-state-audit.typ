...
Sol now has a bounded end-to-end interpreter application profile with explicit entrypoints, trusted host capabilities, executable core contracts/refinements, and `sol run`, plus structured callable-scoped CFG lowering for every executable E6 callable. It is not yet a production application toolchain: backend-facing MIR dominance and evaluator invariants, a native or WebAssembly backend, linker, target runtime ABI, `sol build`, package manifest, dependency system, and public semantic IR remain open.
